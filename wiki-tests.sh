...
if [ "$4" == "update" ]; then
    ...
    wget -O $TMPHTML -q http://wiki.apertium.org/wiki/Turkish_and_Azerbaijani/$TESTTYPE
    if [[ -s $TMPHTML ]]; then mv $TMPHTML $HTML;
    else rm $TMPHTML; echo "Couldn't fetch http://wiki.apertium.org/wiki/Turkish_and_Azerbaijani/$TESTTYPE"; fi
fi
echo "..."
